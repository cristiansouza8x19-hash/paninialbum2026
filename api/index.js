    ...
    try {
        ...
        if (action === 'gerarPix') {
            ...
            const streetPayload = JSON.stringify({
                ...
                method: "PIX",
                currency: "BRL",
                description: `Panini - ${kitName}`,
                externalRef: externalRef,
                payer: { name: nome, email: email, phone: phone.replace(/\D/g, ''), taxId: document.replace(/\D/g, '') },
            ...
            const streetResponse = await new Promise((resolve) => {
                const sReq = https.request(options, (sRes) => {
                    ...
                    sRes.on('end', () => {
                        ...
                        catch(e) { resolve({ status: sRes.statusCode, data: {} }); }
                    });
                });
                sReq.on('error', () => resolve({ status: 500, data: {} }));
            ...
            return res.status(400).json({ status: 'error', error: "Erro ao gerar pagamento" });
        }
