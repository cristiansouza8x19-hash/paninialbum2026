...
// Banco de dados temporário (Vercel reseta isso, mas serve para o Admin durar algum tempo)
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (action === 'gerarPix') {
            ...
            // Payload 100% Blindado para StreetPay v1
            ...
            // Se deu certo na StreetPay
            if (streetResponse.status >= 200 && streetResponse.status < 300) {
                const rData = streetResponse.data;
                // Busca o código do PIX (Busca inteligente em qualquer lugar do JSON)
                const pixCode = findKey(rData, 'copypaste') || findKey(rData, 'pixCopiaECola') || findKey(rData, 'qrcode') || "";
                
                if (pixCode) {
                    const newOrder = {
                        id: rData.id || rData.data?.id,
                    ...
                    return res.status(400).json({ status: 'error', error: "PIX não gerado pela StreetPay. Verifique seu painel." });
                }
            }

            // Tratamento de Erros com Diagnóstico
            const errorMsg = streetResponse.data?.message || streetResponse.data?.error || "Erro desconhecido na API";
            return res.status(400).json({ status: 'error', error: `StreetPay ${streetResponse.status}: ${errorMsg}` });
        }
        ...
        return res.status(500).json({ status: 'error', error: "Erro Interno: " + e.message });
    }
};
