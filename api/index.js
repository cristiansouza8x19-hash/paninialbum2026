    ...
    try {
        ...
        if (action === 'gerarPix') {
            ...
            const streetPayload = JSON.stringify({
                ...
                payer: { name: nome, email: email, phone: cleanPhone, taxId: cleanCpf },
                items: [{ quantity: 1, name: kitName, price: valorFinal, type: "PHYSICAL" }],
                delivery: {
                    fee: 0,
                    address: {
            ...
            if (streetResponse.status >= 200 && streetResponse.status < 300) {
                ...
                // EXTRAÇÃO BLINDADA: Tenta ler de todos os formatos possíveis que a StreetPay usa
                const pixCode = data.data?.copypaste || data.copypaste || data.pixCopiaECola || data.qrcode || "";
                
                if (!pixCode) {
                    console.error("DEBUG: Resposta sem PIX:", JSON.stringify(data));
                    return res.status(400).json({ status: 'error', error: "A StreetPay criou a venda, mas não enviou o código do PIX. Verifique seu saldo ou configurações na StreetPay." });
                }
